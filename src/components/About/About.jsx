        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-white px-6 text-center">
          <div>
            ...
            <p className="text-blue-200 mt-1 text-lg">
              {COMMUNITY.university} · Est. {COMMUNITY.founded}
            </p>
          </div>
      ...
      <div className="w-full max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-10">
          {STATS.map((stat, i) => (
            <FadeIn key={stat.label} delay={i * 80}>
              <div
                className="bg-white rounded-2xl shadow-md p-6 text-center border-2 border-transparent hover:border-blue-500 transition-all w-40 md:w-55 cursor-pointer"
                onClick={() => {
        ...
        <FadeIn>
          <div className="flex justify-end mt-4">
            <button
              className=" group border-2 hover:border-blue-400 w-50 h-12 rounded-2xl bg-dblue font-black text-white p-2 transition-all cursor-pointer "
              onClick={() => {
                navigate("/teams");
              }}
            >
